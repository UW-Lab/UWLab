...
@configclass
class StageSpecificPreviewSurfaceCfg(VisualMaterialCfg):
    """Configuration parameters for creating a preview surface.

    Different from the IsaacLab version, this uses pxr UsdShadeShader with UsdPreviewSurface attributes
    to create a preview surface instead of using the USD command `CreatePreviewSurfaceMaterialPrim`,
    which relies on prim created through isaac sim opened usd stage.

    See :meth:`spawn_preview_surface` for more information.
    """

    func: Callable = visual_materials.stage_specific_spawn_preview_surface
...
@configclass
class PxrMdlFileCfg(VisualMaterialCfg):
    """Configuration parameters for loading an MDL material from a file.

    Different from the IsaacLab version, this uses pxr libraries to create a preview surface instead of
    using the USD command `CreateMdlMaterialPrim`, which only creates prim under isaac sim opened usd stage.

    See :meth:`spawn_from_mdl_file` for more information.
    """

    func: Callable = visual_materials.stage_specific_spawn_from_mdl_file
