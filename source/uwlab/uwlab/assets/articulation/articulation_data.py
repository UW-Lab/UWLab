...
class ArticulationData:
    ...
    def __init__(self, view: ArticulationView, device: str):
        ...
        self.view: ArticulationView = weakref.proxy(view)

        # Set initial time stamp
        self._timestamp = 0.0

        # Convert to direction vector
        gravity_dir = torch.tensor((0.0, 0.0, -9.8), device=self.device)
        gravity_dir = math_utils.normalize(gravity_dir.unsqueeze(0)).squeeze(0)

        # Initialize constants
        self.GRAVITY_VEC_W = gravity_dir.repeat(self.view.count, 1)
        self.FORWARD_VEC_B = torch.tensor((1.0, 0.0, 0.0), device=self.device).repeat(self.view.count, 1)

        # Initialize history for finite differencing
        self._previous_joint_vel = self.view.get_dof_velocities().clone()
    ...
    def update_articulations_kinematic(self):
        """Update the kinematic state of all articulations."""
        pass
    ...
    def root_state_w(self):
        ...
        if self._root_state_w.timestamp < self._timestamp:
            # read data from simulation
            pose = self.view.get_root_transforms().clone()
            pose[:, 3:7] = math_utils.convert_quat(pose[:, 3:7], to="wxyz")
            velocity = self.view.get_root_velocities()
            # set the buffer data and timestamp
            self._root_state_w.data = torch.cat((pose, velocity), dim=-1)
            self._root_state_w.timestamp = self._timestamp
    ...
    def root_link_state_w(self):
        ...
        world.
        """
        if self._root_link_state_w.timestamp < self._timestamp:
            # read data from simulation
            pose = self.view.get_root_transforms().clone()
            pose[:, 3:7] = math_utils.convert_quat(pose[:, 3:7], to="wxyz")
            velocity = self.view.get_root_velocities().clone()
            ...
            self._root_link_state_w.timestamp = self._timestamp
    ...
    def root_com_state_w(self):
        ...
        """
        if self._root_com_state_w.timestamp < self._timestamp:
            ...
            self._root_com_state_w.timestamp = self._timestamp
    ...
    def body_state_w(self):
        ...
        if self._body_state_w.timestamp < self._timestamp:
            self.update_articulations_kinematic()
            # read data from simulation
            poses = self.view.get_link_transforms().clone()
            poses[..., 3:7] = math_utils.convert_quat(poses[..., 3:7], to="wxyz")
            velocities = self.view.get_link_velocities()
            # set the buffer data and timestamp
            self._body_state_w.data = torch.cat((poses, velocities), dim=-1)
            self._body_state_w.timestamp = self._timestamp
    ...
    def body_link_state_w(self):
        ...
        """
        if self._body_link_state_w.timestamp < self._timestamp:
            self.update_articulations_kinematic()
            # read data from simulation
            pose = self.view.get_link_transforms().clone()
            pose[..., 3:7] = math_utils.convert_quat(pose[..., 3:7], to="wxyz")
            velocity = self.view.get_link_velocities()
            ...
            self._body_link_state_w.timestamp = self._timestamp
    ...
    def body_com_state_w(self):
        ...
        principle inertia.
        """
        if self._body_com_state_w.timestamp < self._timestamp:
            self.update_articulations_kinematic()
            # read data from simulation (pose is of link)
            pose = self.view.get_link_transforms().clone()
            pose[..., 3:7] = math_utils.convert_quat(pose[..., 3:7], to="wxyz")
            ...
            self._body_com_state_w.timestamp = self._timestamp
    ...
    def body_acc_w(self):
        ...
        """
        if self._body_acc_w.timestamp < self._timestamp:
            # read data from simulation and set the buffer data and timestamp
            self._body_acc_w.data = self.view.get_link_accelerations()

            self._body_acc_w.timestamp = self._timestamp
    ...
    def joint_pos(self):
        ...
        if self._joint_pos.timestamp < self._timestamp:
            # read data from simulation and set the buffer data and timestamp
            self._joint_pos.data = self.view.get_dof_positions()
            self._joint_pos.timestamp = self._timestamp
        return self._joint_pos.data

    @property
    def joint_vel(self):
        """Joint velocities of all joints. Shape is (num_instances, num_joints)."""
        if self._joint_vel.timestamp < self._timestamp:
            # read data from simulation and set the buffer data and timestamp
            self._joint_vel.data = self.view.get_dof_velocities()
            self._joint_vel.timestamp = self._timestamp
        return self._joint_vel.data

    @property
    def joint_acc(self):
        """Joint acceleration of all joints. Shape is (num_instances, num_joints)."""
        if self._joint_acc.timestamp < self._timestamp:
            # note: we use finite differencing to compute acceleration
            time_elapsed = self._timestamp - self._joint_acc.timestamp
            self._joint_acc.data = (self.joint_vel - self._previous_joint_vel) / time_elapsed
            self._joint_acc.timestamp = self._timestamp
    ...
    def root_link_pos_w(self) -> torch.Tensor:
        ...
        if self._body_com_state_w.timestamp < self._timestamp:
            # read data from simulation (pose is of link)
            pose = self.view.get_root_transforms()
            return pose[:, :3]
    ...
    def body_link_pos_w(self) -> torch.Tensor:
        ...
        if self._body_link_state_w.timestamp < self._timestamp:
            self.update_articulations_kinematic()
            # read data from simulation
            pose = self.view.get_link_transforms()
            return pose[..., :3]
    ...
    def com_pos_b(self) -> torch.Tensor:
        ...
        """
        return self.view.get_coms().to(self.device)[..., :3]
    ...
    def com_quat_b(self) -> torch.Tensor:
        ...
        """
        quat = self.view.get_coms().to(self.device)[..., 3:7]
