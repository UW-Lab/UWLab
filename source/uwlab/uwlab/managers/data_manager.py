...
class DataTerm(ManagerTermBase):
    ...
    A data term is used to generate data for goal-conditioned tasks. For example,
    in the case of a goal-conditioned navigation task, the data term can be used to
    ...
    def __init__(self, cfg: DataTermCfg, env: ManagerBasedRLEnv):
        ...
        Args:
            cfg: The configuration parameters for the data generator.
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> dict[str, float]:
        """Reset the data generator and log infos.
...
class DataManager(ManagerBase):
    ...
    The data manager is used to generate data for an agent to execute. It makes it convenient to switch
    between different data generation strategies within the same environment. For instance, in an environment
    consisting of a quadrupedal robot, the data to it could be a velocity data or position data.
    By keeping the data generation logic separate from the environment, it is easy to switch between different
    data generation strategies.

    The data terms are implemented as classes that inherit from the :class:`DataTerm` class.
    Each data generator term should also have a corresponding configuration class that inherits from the
