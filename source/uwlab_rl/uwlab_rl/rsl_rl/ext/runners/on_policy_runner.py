...
from ..algorithms.extensible_ppo import PPO
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if "offline_algorithm_cfg" in self.alg_cfg:
            if "behavior_cloning_cfg" in self.alg_cfg["offline_algorithm_cfg"]:
                # this is used by the symmetry function for handling different observation terms
                self.alg_cfg["offline_algorithm_cfg"]["behavior_cloning_cfg"]["_env"] = env
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if self.alg.offline:
            if self.alg.offline_bc:
                self.writer.add_scalar("Loss/offline_bc", locs["offline_bc_loss"], locs["it"])
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            # -- For BC
            if self.alg.bc:
                log_string += f"""{'Behavior cloning loss:':>{pad}} {locs['mean_bc_loss']:.4f}\n"""
            ...
            # -- For Offline
            if self.alg.offline:
                log_string += f"""{'Replay Capacity:':>{pad}} {locs['capacity_percentage']:.4f}\n"""
                if self.alg.offline_bc:
                    log_string += f"""{'Offline BC loss:':>{pad}} {locs['offline_bc_loss']:.4f}\n"""
