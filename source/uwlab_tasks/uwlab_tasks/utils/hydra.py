...
def hydra_task_compose(task_name: str, agent_cfg_entry_point: str, hydra_args: list) -> Callable:
    """Decorator to compose Hydra configuration programmatically for a task.

    Unlike :func:`hydra_task_config`, this variant does not rely on ``@hydra.main``.
    It registers the task in Hydra's ConfigStore and uses ``hydra.compose`` with the
    provided overrides to build the configuration tree. Variant-group selections are
    resolved, gymnasium spaces and Python slices are converted to supported types,
    and the decorated function is invoked with materialized configuration objects.

    The decorated function is called as ``func(env_cfg, agent_cfg, *args, **kwargs)``
    and its return value is propagated to the caller.

    Args:
        task_name: Registry name of the task (the last token after ":" is used).
        agent_cfg_entry_point: Registry key to load the agent configuration; pass
            ``None`` or an empty string to skip loading an agent config.
        hydra_args: List of Hydra override strings (e.g.,
            ``["env.physics.dt=0.01", "agent=ppo"]``) applied during composition.

    Returns:
        A decorator that composes the config with the given overrides and calls the
        wrapped function, returning its result.
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            with initialize(config_path=None, version_base="1.3"):
                ...
                # update the group configs with Hydra command line arguments
                runtime_args = {term.split("=")[0]: term.split("=")[1] for term in hydra_args}
                resolve_hydra_group_runtime_override(env_cfg, agent_cfg, hydra_cfg, runtime_args)
                # update the configs with the Hydra command line arguments
