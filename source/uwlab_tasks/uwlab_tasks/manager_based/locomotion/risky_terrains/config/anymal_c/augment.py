...
def aug_func(obs=None, actions=None, env=None, obs_type=Literal["policy", "critic"]):
    aug_obs = None
    aug_act = None
    if obs is not None:
        if obs_type in ("policy", "critic"):
            ...
            raise ValueError(f"unsupported obs type: {obs_type}, only support `policy` or `critic`")
