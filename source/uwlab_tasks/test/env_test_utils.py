...
"""Shared test utilities for UW Lab environments."""
...
def setup_environment(
    ...
    for task_spec in gym.registry.values():
        ...
        if not ("UW" in task_spec.id or "UW" in task_spec.id or "OmniReset" in task_spec.id):
            continue

        # skip Deployment environments
        if "Deploy" in task_spec.id:
            continue

        # skip PegInsert because somehow the asset loading has issue with running with test_environments.rst
        # but it is fine if run normal rl scripts
        if "PegInsert" in task_spec.id:
            continue
