...
Assets used in UW Lab are hosted on AWS S3 buckets on the cloud.
...
On the top right of the UW Lab or Isaac Sim app, look for the icon labeled ``CACHE:``.
...
Hub automatically manages local caching for UW Lab assets, so subsequent runs will use cached files instead of
...
.. note::
   The first time you run UW Lab, assets will still need to be pulled from the cloud, which could lead
