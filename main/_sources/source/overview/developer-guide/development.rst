...
Each extension in UW Lab is written as a python package and follows the following structure:
...
.. code-block:: bash
   ...
   python tools/install_deps.py all ${UWLAB_PATH}/source
...
such cases, we require direct control of the simulation, and so it is necessary to write a standalone application. These applications are functionally similar in that they launch the simulator using the :class:`~uwlab.app.AppLauncher` and
