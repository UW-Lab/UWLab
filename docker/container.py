...
def parse_cli_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Utility for using Docker with UW Lab.")
    ...
    subparsers.add_parser(
        "enter", help="Begin a new bash process within an existing UW Lab container.", parents=[parent_parser]
    )
...
def main(args: argparse.Namespace):
    ...
    if not shutil.which("docker"):
        raise RuntimeError(
            ...
            "https://uw-lab.github.io/UWLab/source/deployment/docker.html"
        )
