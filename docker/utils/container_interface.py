...
class ContainerInterface:
    """A helper class for managing UW Lab containers."""
    ...
    def __init__(
        ...
        Args:
            ...
            suffix: Optional docker image and container name suffix.  Defaults to None, in which case, the docker name
                ...
                passed to suffix, then the produced docker image and container will be named ``uw-lab-base-custom``.
        """
        ...
        if self.profile == "uwlab":
            # Silently correct from uwlab to base, because uwlab is a commonly passed arg
        ...
        self.container_name = f"uw-lab-{self.profile}{self.suffix}"
        self.image_name = f"uw-lab-{self.profile}{self.suffix}:latest"
    ...
    def start(self):
        ...
        container_history_file = self.context_dir / ".uw-lab-docker-history"
    ...
    def copy(self, output_dir: Path | None = None):
        ...
        if self.is_container_running():
            ...
            docker_isaac_lab_path = Path(self.dot_vars["DOCKER_UWLAB_PATH"])
            artifacts = {
            ...
            for container_path, host_path in artifacts.items():
                subprocess.run(
                    [
                        "docker",
                        "cp",
                        f"uw-lab-{self.profile}{self.suffix}:{container_path}/",
                        f"{host_path}",
                    ],
                    check=False,
                )
