...
.. code-block:: bash
    ...
    ./uwlab.sh -p tools/run_all_tests.py

    # for dry run
    ./uwlab.sh -p tools/run_all_tests.py --discover_only

    # for quiet run
    ./uwlab.sh -p tools/run_all_tests.py --quiet

    # for increasing timeout (default is 600 seconds)
    ./uwlab.sh -p tools/run_all_tests.py --timeout 1000
...
def parse_args() -> argparse.Namespace:
    ...
    default_test_dir = os.path.join(UWLAB_PATH, "source")
    ...
    default_log_path = os.path.join(UWLAB_PATH, "logs", "test_results", log_file_name)
...
def warm_start_app():
    ...
    warm_start_output = subprocess.run(
        [
            sys.executable,
            "-c",
            "from uwlab.app import AppLauncher; app_launcher = AppLauncher(headless=True); app_launcher.app.close()",
        ],
        capture_output=True,
    )
    ...
    warm_start_rendering_output = subprocess.run(
        [
            ...
            "-c",
            (
                "from uwlab.app import AppLauncher; app_launcher = AppLauncher(headless=True,"
